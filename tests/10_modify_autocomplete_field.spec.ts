...
test('Modify autocomplete text box', async ({ cvReformatterUploadPage, validationPage }) => {
  await cvReformatterUploadPage.clickOnSelectAndValidate("24.0%");
  await validationPage.isValidationPagePresent();
  await validationPage.validateLastNameField("Jonshon");
